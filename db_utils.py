...
CREATE TABLE IF NOT EXISTS operation_costs (
    id                  BIGSERIAL PRIMARY KEY,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    bot_operation_id    BIGINT NOT NULL REFERENCES bot_operations(id) ON DELETE CASCADE,

    cost_type           TEXT NOT NULL,         -- 'exchange_fee', 'gas_fee', 'api_tokens', 'tax_estimate', ecc.
    amount_usd          NUMERIC(20, 8) NOT NULL,
    currency            TEXT NOT NULL DEFAULT 'USD',

    -- Dettagli opzionali, dipende dal tipo di costo
    exchange            TEXT,                  -- es. 'hyperliquid'
    model_name          TEXT,                  -- es. 'gpt-5.1'
    tokens_input        BIGINT,
    tokens_output       BIGINT,
    fee_rate            NUMERIC(10, 6),        -- es. 0.0005 = 0.05%
    tax_year            INTEGER,
    tax_regime          TEXT,                  -- es. 'IT_simulazione'

    raw_payload         JSONB                  -- per salvare la risposta grezza di exchange / API se vuoi
);

CREATE INDEX IF NOT EXISTS idx_operation_costs_bot_operation_id
    ON operation_costs(bot_operation_id);

CREATE INDEX IF NOT EXISTS idx_operation_costs_cost_type
    ON operation_costs(cost_type);
"""
...
def log_operation_cost(
    ...
    *,
    ...
    """
    Inserisce un costo associato a una bot_operation nella tabella operation_costs.

    Esempi di cost_type:
    - 'exchange_fee'   -> fee pagata sull'exchange (es. Hyperliquid)
    - 'gas_fee'        -> eventuale costo di rete
    - 'api_tokens'     -> costo della chiamata al modello LLM (OpenAI, ecc.)
    - 'tax_estimate'   -> stima di tassazione (simulazione per la tesi)

    Il campo raw_payload può contenere il JSON grezzo di risposta dell'exchange o dell'API.
    """
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (
                    ...
                    tax_year,
                    tax_regime,
                    Json(_normalize_for_json(raw_payload)) if raw_payload is not None else None,
                ),
            )
...
def log_exchange_fee(
    ...
    *,
    ...
    """
    Logga una fee di exchange (es. commissione di Hyperliquid) legata a una bot_operation.
    ...
    """
    return log_operation_cost(
        bot_operation_id=bot_operation_id,
        cost_type="exchange_fee",
...
def log_api_cost(
    ...
    *,
    ...
    """
    Logga il costo di una chiamata API al modello LLM (OpenAI o altro).
    ...
    Esempio d'uso:
        log_api_cost(
            ...
            tokens_input=1200,
            tokens_output=350,
            raw_payload=openai_response_json
        )
    """

    return log_operation_cost(
        bot_operation_id=bot_operation_id,
        cost_type="api_tokens",
        amount_usd=amount_usd,
        currency=currency,
...
def log_tax_estimate(
    bot_operation_id: int,
    *,
    amount_usd: float,
    tax_year: int,
    tax_regime: str = "IT_simulazione",
    raw_payload: Optional[Dict[str, Any]] = None,
    currency: str = "USD",
) -> int:
    """
    Logga una stima di tassazione (simulazione accademica) legata a una bot_operation.
    ...
    Esempio d'uso:
        log_tax_estimate(
            ...
            amount_usd=15.30,
            tax_year=2025,
            tax_regime="IT_regime_X_simulato",
            raw_payload={"method": "FIFO", "note": "..."}
        )
    """

    return log_operation_cost(
        bot_operation_id=bot_operation_id,
        cost_type="tax_estimate",
        amount_usd=amount_usd,
        currency=currency,
        tax_year=tax_year,
