...
def previsione_trading_agent(prompt):
    ...
    text={
        ...
        "schema": {
            ...
            "leverage": {
                "type": "number",
                "description": "Leverage multiplier (risk/reward, 1-40). Only applicable for 'open'.",
                "minimum": 1,
                "maximum": 40
            },
            "reason": {
...
# 1) JSON dell'operazione (come prima)
    decision = json.loads(response.output_text)

    # 2) Usage per costi modello
    usage = {
    ...
    # 3) Se vuoi tenerti anche il raw response per logging avanzato:
    return decision, usage, response
    # Se vuoi più semplice:
